...
#error USE_ONLY_ONE_TEST
		...
		Dictionary::Instance d1 = Create((char*)"Преподаватели", 5);
		Dictionary::Entry e1 = { 1,"Плиско" }, e2 = { 2,"Архипенко" }, e3 = { 3,"Колентионок" }, e4 = { 4, "Бабицкий" }, e5 = { 5, "Хадатович" };
		Dictionary::AddEntry(d1, e1);
		Dictionary::AddEntry(d1, e2);
		Dictionary::AddEntry(d1, e3);
		Dictionary::AddEntry(d1, e4);
		Dictionary::Entry getE = GetEntry(d1, 4);
		Dictionary::DelEntry(d1, 2);
		Dictionary::Entry newentry1 = { 6, "Иванов" };
		Dictionary::UpdEntry(d1, 3, newentry1);
		Dictionary::Print(d1);

		Dictionary::Instance d2 = Create((char*)"Студенты", 5);
		Dictionary::Entry s1 = { 1,"Мамаева" }, s2 = { 2,"Белашков" }, s3 = { 4,"Хлыстов" };
		Dictionary::AddEntry(d2, s1);
		Dictionary::AddEntry(d2, s2);
		Dictionary::AddEntry(d2, s3);
		Dictionary::Entry newentry3 = { 3, "Булавский" };
		Dictionary::UpdEntry(d2, 4, newentry3);
		Dictionary::Print(d2);
		Delete(d1);
		Delete(d2);
